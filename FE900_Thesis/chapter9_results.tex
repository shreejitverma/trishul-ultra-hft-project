...
The most critical metric for a market-making system is Tick-to-Trade (T2T) latency, defined as the time elapsed between the arrival of a market data packet at the NIC and the transmission of the corresponding order packet.
...
The median T2T latency of \textbf{850 nanoseconds} represents a significant achievement. In our testing, we look at P50 (the typical delay you see most of the time) and P99 (the worst-case delay that only happens 1\% of the time). In high-speed trading, keeping the worst-case delay low is just as important as being fast on average.
...
The market-making strategy, enhanced with the Order Book Imbalance (OBI) signal, was backtested against a synthetic dataset comprising 10 million trade events generated via a \textbf{Geometric Brownian Motion (GBM)} model, which is a continuous-time stochastic process where the logarithm of the randomly varying quantity follows a Brownian motion with drift.
...
The \textbf{Sharpe Ratio of 1.85} indicates a robust risk-adjusted return. In simple terms, the Sharpe Ratio measures "bang for your buck," representing how much profit you make compared to how much the price swings up and down. The \textbf{Sortino Ratio of 2.10} is a variation that only penalizes "bad" swings (downside volatility), suggesting the strategy effectively avoided large losses. This confirms the efficacy of the OBI signal in detecting short-term liquidity imbalances and adjusting quotes to avoid \textbf{toxic flow}, which refers to order flow from informed traders that consistently moves the price against the market maker.
