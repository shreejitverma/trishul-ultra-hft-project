...
A robust evaluation requires a realistic counterparty. We implemented a fully functional `MatchingEngine` to simulate the exchange. This simulator enforces Price-Time Priority, allowing for realistic assessment of strategy performance. This ensures that the strategy is tested against realistic constraints, including queue position and partial fills.

\subsection{Vectorized Signal Engine}
To support the computational demands of the RL strategy, a `SignalEngine` was developed using \textbf{AVX2 SIMD} intrinsics. AVX2 SIMD intrinsics enable the parallel processing of price vectors at the register level. By processing batches of price data in parallel, the software control plane can recalculate model parameters over millions of historical ticks in under 200 milliseconds.
