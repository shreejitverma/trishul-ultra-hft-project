...
The choice of the SPSC pattern is driven by the unidirectional flow of the trading pipeline. By strictly limiting each queue to a single producer and a single consumer, we eliminate the need for complex multi-writer arbitration or hardware-level atomic CAS (Compare-And-Swap) loops. The SPSC queue utilized in this project (see Appendix \ref{sec:codeexplain}) operates using \textbf{memory barriers} and \textbf{C++20 atomic memory ordering} (\texttt{std::memory\_order\_acquire}/\texttt{release}), which ensures that data produced by one core is visible to the consumer on another core with minimal CPU-cycle overhead.

Key technical advantages of this SPSC implementation include:
\begin{itemize}
    \item \textbf{Contention-Free Design:} Since only one thread modifies the head pointer and another modifies the tail, there is zero resource contention at the hardware level.
    \item \textbf{Cache-Line Padding:} To prevent \textbf{False Sharing}, the head and tail pointers are separated by 64 bytes (the standard x86\_64/ARM cache-line size), ensuring they reside in different cache lines.
    \item \textbf{Determinism:} Without locks, the worst-case execution time of a ``push'' or ``pop'' operation is constant and predictable, a non-negotiable requirement for high-frequency trading.
\end{itemize}
...
The first stage, implemented by the \texttt{MulticastReceiver}, is engineered for the ingestion of raw UDP packets from real-world exchange multicast feeds. \datamethodology\ We simulate a hardware-based kernel bypass (similar to Solarflare's OpenOnload or DPDK) using a \texttt{DMARingBuffer}.
...
\end{equation}
This vectorization provides a theoretical 8x speedup over scalar C++ code, allowing the system to recalculate model features across a full day of exchange data in less than 200 milliseconds. \datamethodology
...
To minimize the speed-of-light propagation delay, the system is designed for deployment in a \textbf{Co-location (Co-lo)} facility, such as Equinix NY4 (Secaucus) or CH2 (Chicago). In these environments, even the physical length of the fiber-optic patch cables becomes a source of competitive advantage. We utilize \textbf{Fiber-Optic Cable Length Parity} to ensure that the time-of-flight for market data arrival and order transmission is symmetric and predictable.

\subsection{Precision Time Protocol (PTP) Synchronization}
To maintain a consistent ``Market View'' across the hybrid FPGA-CPU architecture, the system implements hardware-level \textbf{PTP (Precision Time Protocol)} synchronization.
\begin{itemize}
    \item \textbf{Hardware Timestamping:} The FPGA utilizes a dedicated \textbf{PTP Servo Core} to synchronize its internal 1ns-precision clock with the data center's \textbf{Grandmaster Clock}. Every packet processed by the system is timestamped at the physical layer (PHY) the moment the Start-of-Frame delimiter is detected.
