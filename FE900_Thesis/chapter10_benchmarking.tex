...
\begin{enumerate}
    \item \textbf{Traditional Software Stack:} This baseline represents the standard high-performance trading architecture utilized by most quantitative firms. It consists of optimized C++ code running on general-purpose CPUs, utilizing \textbf{Kernel-Bypass (User-space networking)} and \textbf{Core Pinning} to minimize OS overhead. While highly flexible, it is subject to non-deterministic ``jitter'' caused by L3 cache contention, branch mispredictions, and unavoidable kernel interrupts.
    \item \textbf{Static Hardware Strategy:} This baseline represents an FPGA-based execution engine that uses fixed-logic algorithms (e.g., a simple linear model or a constant-spread rule). It achieves the same nanosecond-level determinism as the proposed system but lacks the \textbf{adaptive intelligence} of reinforcement learning. It cannot adjust its quoting behavior in response to regime shifts or rising microstructural toxicity (High VPIN).
\end{enumerate}
...
\begin{itemize}
    \item $\mathcal{L}_{fixed}$: The constant propagation delay of FPGA gates and physical medium, which is invariant regardless of market load.
...
\end{equation}
A higher $\mathcal{U}$ indicates that the system is more confident in its risk management, allowing it to deploy more capital without increasing the probability of ruin. This relationship is further explored in Figure \ref{fig:drawdown_comparison_bench}.
...
The most significant limitation occurs during extreme ``market data bursts'' characterized by a high volume of small packets. \datamethodology\ As shown in the updated Figure \ref{fig:throughput_bench_ch8}, the system maintains constant nanosecond latency for the first 75 million events. 
However, as the burst intensity increases and the cumulative volume approaches 90 million events, we observe the onset of the \textbf{PCIe/DMA Saturation Bound}. The physical transaction-per-second (TPS) limit of the PCIe Gen4 x16 bus causes the FPGA's internal \textbf{Burst Buffers} to reach full capacity. This induces an aggressive ``Slowdown Point'' where Tick-to-Trade latency spikes from 860ns to over \textbf{400,000ns} (400 $\mu$s) due to hardware backpressure. This empirical result proves that while the system is ultra-low-latency, its determinism is bounded by the physical drain rate of the host interconnect.

\subsection{Resource Constraints and Model Complexity}
The deterministic nature of the RL inference core is dependent on the available \textbf{DSP48 slices} and \textbf{Block RAM (BRAM)} on the Kintex UltraScale+ FPGA. 
\begin{equation}
